...
def upload_pdf_route():
    try:
        ...
        if not rows:
            return jsonify({"error":"Could not parse PDF - check Railway logs"}),400
        # Clear existing PDF data for this month
        conn = get_db()
...
app.add_url_rule("/api/upload-pdf","upload_pdf_fn",upload_pdf_route,methods=["POST"])

def reset_month_route():
...
app.add_url_rule("/api/reset-month","reset_month_fn",reset_month_route,methods=["GET"])
...
app.add_url_rule("/api/data","data_fn",data_route)
app.add_url_rule("/health","health_fn",lambda:jsonify({"status":"ok","db":DB_PATH}))
...
app.add_url_rule("/","index_fn",index_route)

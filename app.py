...
const ca=d.cards.length?`<div class="grid">${d.cards.map(c=>{return`<div class="card"><div class="card-title">${c.label}</div><div class="card-row"><span class="card-amount" style="color:${c.color}">S${(c.cost*1.28).toFixed(2)}</span></div><div class="bar-bg" style="margin-top:10px"><div class="bar-fill" style="width:${c.pct_of_total}%;background:${c.color}"></div></div><div class="card-footer"><span class="card-pct">${c.pct_of_total}% of total</span><span class="card-pct">${c.message_count.toLocaleString()} units</span></div></div>`;}).join("")}</div>`:`<div class="empty"><h2>No data</h2></div>`;
el.innerHTML=`<div class="disclaimer">⚠️ <span><b>Disclaimer:</b> This billing report reflects your actual GHL credit usage for the selected month. Reports are updated on the <b>7th of each month</b> once all charges have been finalized by GoHighLevel. Data shown prior to the 7th may be incomplete or subject to change.</span></div><div class="plan-card"><div class="plan-left"><div class="plan-icon">📋</div><div><div class="plan-name">Professional Plan</div><div class="plan-sub">Engages.ai Subscription</div></div></div><div><span class="plan-price">S$380<span class="plan-price-lbl">/month</span></span></div></div><div class="plan-card"><div class="plan-left"><div class="plan-icon">💳</div><div><div class="plan-name">Credit Spending</div><div class="plan-sub">Total for ${fmtMonth(active)}</div></div></div><div><span class="plan-price">S${(d.total*1.28).toFixed(2)}<span class="plan-price-lbl">/month</span></span></div></div>${ca}`;
}

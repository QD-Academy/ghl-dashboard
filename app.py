...
def parse_pdf(pdf_bytes):
    try:
        ...
        from pdfminer.high_level import extract_text
        text = extract_text(io.BytesIO(pdf_bytes))
    print(f"PDF: {len(text)} chars extracted", flush=True)
    # Service patterns ordered most-specific first
    SVCS = [
    ...
    while i < len(lines):
        ...
        svc = None
        for pat, key in SVCS:
            if pat in ll and "item name" not in ll and "unit price" not in ll and "total" not in ll:
                svc = key
                break
        if svc and svc not in seen:
            ...
            if total > 0:
                ...
                seen.add(svc)
                print(f"  {svc}: qty={qty} cost={total}", flush=True)
        i += 1
    return results
...
app.add_url_rule("/api/upload-pdf","upload_pdf",upload_pdf_route,methods=["POST"])
...
def run_fetch():
    ...
    for yr,mo in months:
        ...
        conn.close()
        if has_pdf>0:
            print(f"  {yr}-{mo:02d}: skipping (PDF exists)",flush=True); continue
        ...
        while offset<5000:
            ...
            if not convs: break
            for cv in convs:
                svc={"type_whatsapp":"whatsapp_marketing","type_email":"email","type_sms":"sms"}.get(
                    (cv.get("lastMessageType","") or "").lower(),"other")
                counts[svc]+=1
            total+=len(convs); offset+=100; time.sleep(0.05)
        month_str=f"{yr}-{mo:02d}"
        for svc,cnt in counts.items():
            upsert(month_str,svc,cnt,0,"api")
...
app.add_url_rule("/health","health",lambda:jsonify({"status":"ok","db":DB_PATH}))

def data_route():
    try:
        months,result=get_api_data()
